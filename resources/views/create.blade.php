...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    <head>
        ...
        <title>Tambah Aktivitas Baru — Through The Lenses.</title>
  ...
  <div id="canvas" class="max-w-2xl mx-auto w-full">
    ...
    <main class="py-4">
      <h1 class="text-4xl font-serif font-bold mb-6">Tambah Aktivitas Baru</h1>
      ...
      <form action="{{ route('activities.store') }}" method="POST" class="space-y-6">
          ...
          <div>
              ...
              <input type="text" name="title" value="{{ old('title') }}" required class="w-full border border-gray-300 rounded p-3 text-sm focus:outline-none focus:border-black">
          </div>

          <div>
              <label class="block text-sm font-semibold uppercase tracking-wider mb-2">Kategori</label>
              <input type="text" name="category" value="{{ old('category') }}" required class="w-full border border-gray-300 rounded p-3 text-sm focus:outline-none focus:border-black" placeholder="Contoh: Website Programming">
          </div>

          <div>
              <label class="block text-sm font-semibold uppercase tracking-wider mb-2">Tanggal Aktivitas</label>
              <input type="date" name="activity_date" value="{{ old('activity_date') }}" required class="w-full border border-gray-300 rounded p-3 text-sm focus:outline-none focus:border-black">
          </div>
          ...
          <div>
              ...
              <textarea name="description" rows="4" required class="w-full border border-gray-300 rounded p-3 text-sm focus:outline-none focus:border-black">{{ old('description') }}</textarea>
          </div>

          <div class="flex justify-end gap-4 pt-4">
              <a href="/" class="px-6 py-3 border border-gray-300 rounded text-sm font-semibold hover:bg-gray-100 transition">Batal</a>
              <button type="submit" class="px-6 py-3 bg-black text-white rounded text-sm font-semibold hover:bg-gray-800 transition">Simpan Aktivitas</button>
          </div>
      </form>
    </main>
